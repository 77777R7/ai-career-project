...
更新：2026-06-27，根据 GPT Pro 评估，V1 onboarding 主线按 Canadian university explorer 设计；高中生、新毕业生和 early-career users 作为 secondary mode，不作为首屏复杂分支。
...
`grade_level` 不是必填字段。高中生可以使用，但不需要被限制为某个具体年级。V1 的首屏问题、默认 examples 和 demo flow 先按大学生 explorer 设计，高中生走轻量 exploration mode。
...
Demo 可以使用 Vancouver 作为 Canadian Housing and Cost of Living Dashboard 的默认城市样例，但产品边界是全加拿大。
...
### 12.5 Advisor Lite 触发补问

Advisor Lite 不应该一次性索要完整 profile。它只在回答需要时补一个最小问题。

| 用户问题 | Advisor Lite 可补问字段 |
| --- | --- |
...
下一步进入 `IMPLEMENTATION_PLAN_ZH.md` 的 Project Scaffold / KB Loader / Mock API 顺序。Supabase / Postgres SQL draft 保留为 Phase 2 persistence 参考，不作为第一轮可跑 demo 的前置。
