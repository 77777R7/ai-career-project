...
更新：2026-06-27，根据 GPT Pro 评估，本文件作为 Phase 2 persistence schema draft；V1 demo 可以先用 mock runtime store 跑通闭环，不要求先接 Supabase、完整 audit logs 或 Basic RAG。
...
> 用户填完 profile 之后，系统如何保存用户状态、career match 结果、roadmap、project progress、Growth Snapshot metrics、Advisor Lite 对话、consent 和后续 audit logs？

本文件承接当前产品顺序：

```text
Student Profile -> Career Path Matcher -> Save Primary Path -> Roadmap -> Project Builder -> Growth Snapshot -> Advisor Lite -> Basic RAG Later
```
...
6. Growth Snapshot 根据真实状态更新。
7. Advisor Lite 根据 profile、match、roadmap、project progress 回答。
...
```text
mock runtime store first; app database later
```
...
- Growth Snapshot metric snapshots。
- Advisor Lite session 和 message。
- Consent records。
- Audit logs。
...
Basic RAG 在 Advisor Lite 和核心 demo loop 跑通后接入。第一版产品表只需要为 Advisor message 留好 `sources_used` / `citation_refs` 字段，不需要一开始做完整 RAG 表。
...
Career Match、Roadmap、Growth Snapshot metric、Advisor message 都应该保存当时的输入摘要和版本号。
...
### 4.6 Advisor Lite 不是事实来源

Advisor Lite 可以解释、总结、建议下一步，但不能覆盖结构化状态。

例如：

- 保存 primary path 必须写入 `saved_paths`。
- 完成 project step 必须写入 `project_step_progress`。
- Growth Snapshot 分数必须写入 `dashboard_metric_snapshots`。
...
| Growth Snapshot | `dashboard_metric_snapshots` | `student_profiles`, `career_matches`, `roadmap_tasks`, `project_step_progress` |
| Advisor Lite | `advisor_sessions`, `advisor_messages` | profile, match, roadmap, project, growth snapshot, KB refs |
...
保存用户当前职业规划 profile。Career Matcher、Roadmap、Growth Snapshot、Advisor Lite 都从这里读取当前状态。

### 写入时机

- Onboarding 完成。
- 用户修改兴趣、技能、目标、项目经历。
- Advisor Lite 或 Growth Snapshot 引导用户补充信息后，用户确认更新。
...
保存 profile 的重要变更历史，用于 audit、growth trend、Advisor Lite 上下文。
...
Growth Snapshot 不应该只显示静态百分比。它应该根据用户真实状态计算，例如：
...
Growth Snapshot 默认读取每个 `metric_id` 最新一条 snapshot。
...
用于记录 Advisor Lite 每次检索了哪些 chunks。
...
- `advisor_sessions`
- `advisor_messages`
- optional `audit_logs` for high-risk or blocked events later
...
高风险或 blocked 输出在 V1 至少要带 risk flag；接 Supabase 后再写入完整 audit log。
...
- 可以把 `project.canadian_housing_cost_living_dashboard` 实例化为用户项目。
- 可以保存 `action.define_project_question` 的完成结果。
- Growth Snapshot 可以读取真实状态并生成 metric snapshots。
- Advisor Lite 可以从结构化状态生成上下文，而不是要求用户重新描述。
- Consent 可以记录关键行为；完整 audit log 后置。

## 16. Open Decisions

第一版实现前还需要确认：

1. 何时从 mock runtime store 切换到 Supabase Auth / Postgres。
2. 是否允许匿名 demo user，不登录也能跑完整 demo。
3. `profile_summary` 是实时生成，还是保存后只在 profile 更新时刷新。
4. Growth Snapshot metric 是每次事件后自动 recompute，还是用户打开 snapshot 时 lazy recompute。
5. Advisor Lite message 是否默认保存完整内容，以及 retention 时间。
...
- 但前端不应该把所有字段一次性问完。
- Onboarding flow 需要决定哪些字段是首屏必问、哪些是 progressive profile、哪些由 Advisor Lite 后续补问。
